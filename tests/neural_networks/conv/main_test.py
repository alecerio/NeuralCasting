...
class TestConv(unittest.TestCase):
    def test_00(self):
        ...
        print_test_header("CONV ONNX", 4)

        # init config file
        name : str = CompilerConfig()['name']
        output_path : str = CompilerConfig()['output_path']
        test_path : str = CompilerConfig()['test_path'] + 'neural_networks/conv/'
        temp_path : str = CompilerConfig()['temp_path']
        path_onnx = test_path + 'conv.onnx'

        # inference onnxruntime
        input0 = np.ones((1, 3, 8, 8), dtype=np.float32)
        input_data = [input0]
