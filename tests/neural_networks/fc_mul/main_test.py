...
class TestFcMul(unittest.TestCase):
    def test_00(self):

        print_test_header("FULLY_CONNECTED-MUL PYTORCH", 4)

        # init config file
        name : str = CompilerConfig()['name']
        output_path : str = CompilerConfig()['output_path']
        test_path : str = CompilerConfig()['test_path'] + 'neural_networks/fc_mul/'
