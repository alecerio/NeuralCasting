...
from config.config import LIB_DIR, TEST_TMP_PATH

def test_linearadd():
    # compute floating point model
    a_float = np.array([-0.2, -0.1, 0.0, 0.1, 0.2], dtype=np.float32)
    b_float = np.array([-0.6, -0.3, 0.0, 0.3, 0.6], dtype=np.float32)
    c_float = linearadd_float(a_float, b_float)
    ...
    c_rec1 = linearadd_quant_float(a_float, b_float, sa, za, sb, zb, sc, zc)
    assert np.allclose(c_rec1, c_float, atol=1e-1)

    # compute quantized model fixed point
    c_rec2 = linearadd_quant_fixed_point(a_float, b_float, sfxa, zfxa, sfxb, zfxb, sfxc, zfxc, Q)
    assert np.allclose(c_rec2, c_float, atol=1e-1)

    # compute test c
    c_rec3 = linearadd_quant_fixed_point_c(a_float, b_float, sfxa, za, sfxb, zb, sfxc, zc, Q)
    assert np.allclose(c_rec3, c_float, atol=1e-1)

def linearadd_float(a: np.array, b: np.array) -> np.array:
    c = np.add(a, b)
    return c

def linearadd_quant_float(a: np.array, b: np.array, sa: float, za: int, sb: float, zb: int, sc: float, zc: int) -> np.array:
...
def linearadd_quant_fixed_point(a: np.array, b: np.array, sfxa: int, zfxa: int, sfxb: int, zfxb: int, sfxc: int, zfxc: int, Q: int) -> np.array:
...
def linearadd_quant_fixed_point_c(a: np.array, b: np.array, sfxa: int, za: int, sfxb: int, zb: int, sfxc: int, zc: int, Q: int):
