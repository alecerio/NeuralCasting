...
class TestCopyOnnx(unittest.TestCase):
    def test_00(self):
        print_test_header("COPY ONNX", 4)

        # init config file
        test_path : str = CompilerConfig()['test_path'] + 'onnxdbg/copy_onnx/'
        temp_path : str = CompilerConfig()['temp_path']
        path_onnx = test_path + 'gru_reimplemented_1.onnx'
        path_onnx_copy = temp_path + 'gru_reimplemented_1.onnx'
        ...
        onnxdbg("copy", src=path_onnx, dst=path_onnx_copy, mdl='copy_model')

        # inference onnxruntime copy
        [outputs_onnx_copy, outputs_shape_onnx_copy] = inference_onnx_runtime(path_onnx_copy, input_data)
        output_onnx_copy = outputs_onnx_copy[0]
        output_shape_onnx_copy = outputs_shape_onnx_copy[0]

        # compare shape
        compare_shape(self, output_shape_onnx, output_shape_onnx_copy, "ONNX", "ONNX COPY")

        # compare results
        compare_results(self, output_onnx, output_onnx_copy, "ONNX", "ONNX COPY", 1e-6)
