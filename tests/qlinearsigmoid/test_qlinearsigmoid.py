...
def test_qlinearsigmoid():
    ...
    # compute torch model
    y_torch = linearsigmoid_torch_float(x)
    
    # compute floating point model
    y = linearsigmoid_float(x)
    ...
    y_rec1 = linearsigmoid_quant_float(x, sx, zx, LUT_MIN, LUT_MAX, LUT_SIZE)
    assert np.allclose(y_rec1, y_torch, atol=1e-1)

    # compute quantized model fixed point
    y_rec2 = linearsigmoid_quant_fixed_point(x, sfxx, zfxx, LUT_MIN, LUT_MAX, LUT_SIZE, Q)
...
def linearsigmoid_float(a: np.array) -> np.array:
    b = 1.0 / (1.0 + np.exp(-a))
    return b

def linearsigmoid_quant_float(a: np.array, sa: float, za: int, LUT_MIN: int, LUT_MAX: int, LUT_SIZE: int) -> np.array:
...
def linearsigmoid_quant_fixed_point(a: np.array, sfxa: int, zfxa: int, LUT_MIN: int, LUT_MAX: int, LUT_SIZE: int, Q: int) -> np.array:
