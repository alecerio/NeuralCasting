...
class QLinearSigmoid(NCastOp):
    ...
    def emit_run_ops(self, config: NCastConfig, graph: onnx.onnx_ml_pb2.GraphProto, ops: List[NCastOp]) -> str:
        ...
        scale_y = set_valid_tensor_identifier(self.onnx_unit.input[3])
        zero_y = set_valid_tensor_identifier(self.onnx_unit.input[4])
        return f"NCAST_QLINEAR_SIGMOID({x}, {y}, {size}, {scale_x}, {zero_x}, {scale_y}, {zero_y});"
