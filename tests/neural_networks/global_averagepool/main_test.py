...
class TestGlobalAveragepool(unittest.TestCase):
    def test_00(self):

        print_test_header("GLOBAL AVERAGEPOOL ONNX", 4)

        # init config file
        name : str = CompilerConfig()['name']
        output_path : str = CompilerConfig()['output_path']
        test_path : str = CompilerConfig()['test_path'] + 'neural_networks/global_averagepool/'
        temp_path : str = CompilerConfig()['temp_path']
        path_onnx = test_path + 'avgpool.onnx'

        # inference onnxruntime
        input0 = list(range(1, 49))
