...
def get_optype(graph, node):
    for value_info in graph.value_info:
        if value_info.name == node.output[0]:
            ...
            #print(f"Il tipo dell'output è: {onnx.TensorProto.DataType.Name(output_type)}")
            return [output_type, type_code]
    return [1, "int8_t"]#raise Exception(f"output name \"{node.output[0]}\" not found")
...
def generate_inference(model_path, file_c, file_h, is_debug=True):
    ...
    for idx, node in enumerate(graph.node):
        if node.op_type == 'QuantizeLinear':
            ...
            #ncast_debug(node.output[0], 1, shape, output_type)
...
def analyze_onnx_graph(model_path):
    ...
    #nodes = topological_sort_onnx(graph)
    ...
    for idx, node in enumerate(graph.node):
        ...
        if node.op_type == 'QuantizeLinear':
            ...
            if adjust_name(node.input[0]) in shapes.keys():
                ...
                print(f"####: {adjust_name(node.input[0])} -> {adjust_name(node.output[0])}")
                print(shapes.keys())
            #ncast_debug(node.output[0], dim0, dim1, output_type)
...
if __name__ == "__main__":
    ...
    #print("#include \"quant.h\"")
    #print("#include \"utils.h\"")
    #print("#include \"squeeze.h\"")
    #print("#include \"qlinear.h\"")
    #print("#include \"qmatmul.h\"")
    #print("#include \"qadd.h\"")
    #print("#include \"qsigmoid.h\"")
    #print("#include \"qmul.h\"")
    #print("#include \"dqlinear.h\"")
    #print("#include \"sub.h\"")
    #print("#include \"tanh.h\"")
    #print("#define NCAST_QADD_FIXED(INPUT_A,SF_A,ZP_A,INPUT_B,SF_B,ZP_B,SF_Y,ZP_Y,OUPUT,SIZE) printf(\"qadd!\");")
    #print("#define NCAST_QSIGMOID(INPUT,SF_X,ZP_X,SF_Y,ZP_Y,OUTPUT,SIZE) printf(\"qsigmoid!\");")
    #print("#define NCAST_QMUL_FIX(INPUT_A,SF_A,ZP_A,INPUT_B,SF_B,ZP_B,SF_Y,ZP_Y,OUPUT,SIZE) printf(\"qmul!\");")
    #print("#define NCAST_DQLINEAR(INPUT,SF_X,ZP_X,OUTPUT,SIZE) printf(\"dqlinear!\");")
    #print("#define NCAST_SUB(INPUT_A,INPUT_B,OUTPUT,SIZE) printf(\"sub!\");")
    #print("#define NCAST_TANH(INPUT,OUTPUT,SIZE) printf(\"tanh!\");")

    file_c += "#include \"" + nn_name + ".h\"\n"
    file_c += "void run_inference(float32_t* in_noisy, float32_t* h1, float32_t* h2) {\n"
    [file_c, file_h] = generate_inference(model_path, file_c, file_h)
    file_c += "}\n"
    generate_file(nn_name + ".c", file_c)

    file_h += "#endif // __"  + nn_name.upper() + "__\n"
    generate_file(nn_name + ".h", file_h)

    #print("int main() {")
    #print("float32_t h1[400];")
    #print("float32_t h2[400];")
    #print("for(int i=0; i<400; i++) { h1[i] = 0.0f; h2[i] = 0.0f; }")
    #print("float32_t in_noisy[257];")
    #print("for(int i=0; i<257; i++) in_noisy[i] = 1.0f;")
