...
from wcet.qlinearsoftmax.wcet_qlinearsoftmax import generate_input_c, generate_input_h, generate_int16_str

def generate_main_patmos(size, Q, acctype):
    f = generate_int16_str
    nn_statement = f"NC_QLSMAX_FXS8(xq,yq,{size},{f()},{f()},{f()},{f()},{Q},{acctype})"
    generate_main_patmos_template(nn_statement, 100)

def qlinearsoftmax_patmos_analysis(name, size, Q, acctype):
    def generate_c_code():
...
if __name__ == '__main__':
    qlinearsoftmax_patmos_analysis(name="qlinear_softmax", size=100, Q=15, acctype="int32_t")
