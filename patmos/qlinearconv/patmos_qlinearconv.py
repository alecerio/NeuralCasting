...
from wcet.qlinearconv.wcet_qlinearconv import generate_input_c, generate_input_h

def generate_main_patmos(KS, CIN, LIN, COUT, LOUT, PAD, DIL, STR, Q):
...
def qlinearconv_patmos_analysis(name, KS, CIN, LIN, COUT, PAD, DIL, STR, Q):
    def generate_c_code():
        LOUT = int(((LIN + 2*PAD - DIL*(KS-1) - 1) / STR + 1))
        generate_main_patmos(KS, CIN, LIN, COUT, LOUT, PAD, DIL, STR, Q)
...
if __name__ == '__main__':
    qlinearconv_patmos_analysis(name="qlinear_conv", KS=1, CIN=4, LIN=5, COUT=3, PAD=0, DIL=1, STR=1, Q=31)
