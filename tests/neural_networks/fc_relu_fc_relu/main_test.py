from .FcReluFcRelu import FcReluFcRelu
import torch
...
class TestFcReluFcRelu(unittest.TestCase):
    def test_00(self):

        print_test_header("FULLY_CONNECTED-RELU-FULLY_CONNECTED-RELU PYTORCH", 2)

        # init config file
        name : str = CompilerConfig()['name']
        output_path : str = CompilerConfig()['output_path']
        test_path : str = CompilerConfig()['test_path'] + 'neural_networks/fc_relu_fc_relu/'
        temp_path : str = CompilerConfig()['temp_path']
        params_path = test_path + 'params.pth'

        # create pytorch model
        model = FcReluFcRelu(2, 4, 3)
