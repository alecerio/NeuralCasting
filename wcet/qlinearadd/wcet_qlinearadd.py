...
def generate_main_wcet(size, acctype):
    f = generate_int16_str
    nn_statement = f"NC_QLADD_FXS8(aq,bq,cq,{size},{f()},{f()},{f()},{f()},{f()},{f()},{acctype})"
...
def qlinearadd_wcet_analysis(name, size, acctype):
    def generate_c_code():
        generate_main_wcet(size, acctype)
        generate_input_c(size)
        generate_input_h(size)
    generate_wcet_analysis(name, generate_c_code)

if __name__ == '__main__':
    qlinearadd_wcet_analysis(name='qlinearadd_test', size=100, acctype="int32_t")
