...
from wcet.qlinearmul.wcet_qlinearmul import generate_input_c, generate_input_h, generate_int16_str

def generate_main_patmos(size, Q, acctype):
    f = generate_int16_str
    nn_statement = f"NC_QLMUL_FXS8(aq,bq,cq,{size},{f()},{f()},{f()},{f()},{f()},{f()},{Q},{acctype})"
    generate_main_patmos_template(nn_statement, 100)

def qlinearmul_patmos_analysis(name, size, Q, acctype):
    def generate_c_code():
...
if __name__ == '__main__':
    qlinearmul_patmos_analysis(name="qlinear_mul", size=100, Q=15, acctype="int32_t")
